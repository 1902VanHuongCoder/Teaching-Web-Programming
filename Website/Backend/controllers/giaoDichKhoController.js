...
export const nhanTatCaGiaoDichKho = async (req, res) => {
    try {
        ...
        res.status(500).json({ message: "Lỗi khi lấy giao dịch kho", error: error.message });
    }
};
...
export const taoGiaoDichKho = async (req, res) => {
    try {
        const { loaiGiaoDich, ngayGiaoDich, tenSanPham, soLuong, nguoiThucHien, ghiChu, sachID, giaNhap, giaBan } = req.body; 
        ...
        res.status(500).json({ message: "Lỗi khi tạo giao dịch kho", error: error.message });
    }
};
...
export const xoaGiaoDichKho = async (req, res) => {
    try {
        ...
        res.status(500).json({ message: "Lỗi khi xóa giao dịch kho", error: error.message });
    }
};
