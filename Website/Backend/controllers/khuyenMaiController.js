...
export const capNhatMaKhuyenMai = async (req, res) => {
    try {
        ...
        const { giaTriGiam, moTa, ngayHetHan, soLuong, giaCoBan, trangThai } = req.body;
...
export const nhanMaKhuyenMaiTheoID = async (req, res) => {
    try {
        ...
        const khuyenMai = await KhuyenMai.findByPk(khuyenMaiID);
        if (!khuyenMai) {
            return res.status(404).json({ message: "Khuyến mãi không tồn tại", success: false });
        }
        res.status(200).json({ message: "Lấy khuyến mãi thành công", success: true, khuyenMai: khuyenMai });
    } catch (error) {
