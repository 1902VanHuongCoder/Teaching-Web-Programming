...
// Hàm nhận tất cả danh sách người dùng 
// Hàm để mình kiểm tra mã OTP khi yêu cầu đặt lại mật khẩu
// Hàm để gửi email xác nhận đặt lại mật khẩu 
...
export const thayDoiTrangThaiTaiKhoan = async (req, res) => {
    ...
    try {
        ...
        res.status(200).json({ message: "Thay đổi trạng thái tài khoản thành công", user });
    } catch (error) {
        console.error("Lỗi khi thay đổi trạng thái tài khoản:", error);
        res.status(500).json({ message: "Lỗi xảy ra khi cập nhật trạng thái tài khoản" });
    }
};
